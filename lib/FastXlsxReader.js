...
class FastXlsxReader {
  /**
   * Initialize a new instance of the FastXlReader class.
  ...
  _readRow(row, index, schema, lowerCase, onRecord, onError, useMemoryForItems) {
    let record;
  ...
  static iterate(input, sheetname, callback, eventNames, thisArg) {
    ...
    try {
      ...
      for (let row_index = range.s.r; row_index <= range.e.r; row_index++) {
        const row = [];
        for (let col_index = range.s.c; col_index <= range.e.c; col_index++) {
          ...
          */
          if (!!cell) {
            row.push(cell.v);
            if (hasCellCb) callback.call(thisArg, "cell", cell, row_index, col_index);
          }
        }
        rowCount++;
